...
namespace assembly_logs_parser
{
    internal class Program
    {
       ...
       static string statisticConferencesFolderName = "Статистика";
        ...
        {
            List<string> logsFilesPathsList = make_logs_files_paths_list();
            ushort logFileCounter = 0; // счётчик логфайлов

            // сканируем каждый лог файл
            foreach (string logsFilePath in logsFilesPathsList) 
            {
                logFileCounter++;
                add_to_main_log(String.Format("сканирую {0} [{1}/{2}]", Path.GetFileName(logsFilePath), logFileCounter, logsFilesPathsList.Count));
                string[] logFileLines = File.ReadAllLines(logsFilePath); // загружаем все строчки из лог файла

                // сканируем каждую строчку из лог файла
                foreach (string logFileLine in logFileLines) 
                {
                    LogFileLineClass currentLogFileLine = new LogFileLineClass(logFileLine);
        ...
        //private static void scan_logs_temp()
        //{
        ...
        //            LogFileLineClass log_file_line_ = new LogFileLineClass(log_file_line);
        ...
        //                    processed_conf_files_paths.Remove(conf_id);
